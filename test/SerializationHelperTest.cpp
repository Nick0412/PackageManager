...
// TEST(SerializationHelper, Unsigned32BitIntSerialize)
// {
//     std::uint32_t number{0xABCDEF00};
//     std::vector<std::byte> number_bytes = SerializationHelper::Serialize32BitUnsignedInt(number);

//     EXPECT_EQ(std::to_integer<std::uint32_t>(number_bytes[0]), 0x00);
//     EXPECT_EQ(std::to_integer<std::uint32_t>(number_bytes[1]), 0xEF);
//     EXPECT_EQ(std::to_integer<std::uint32_t>(number_bytes[2]), 0xCD);
//     EXPECT_EQ(std::to_integer<std::uint32_t>(number_bytes[3]), 0xAB);
